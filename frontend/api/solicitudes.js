/**
 * solicitudes.js - Study Request API Handler (Vercel Serverless Function)
 * 
 * Handles all operations related to radiological study requests (solicitudes).
 * Patients can create requests publicly, while doctors can manage their received requests.
 * 
 * Endpoints:
 * ---------------------------------------------------------
 * | Method | Params                    | Auth Required | Description                    |
 * |--------|---------------------------|---------------|--------------------------------|
 * | POST   | -                         | No            | Create new study request       |
 * | GET    | -                         | Yes           | Get all requests for doctor    |
 * | GET    | action=pending-count      | Yes           | Get count of pending requests  |
 * | GET    | id=xxx                    | Yes           | Get specific request           |
 * | PUT    | id=xxx&action=accept      | Yes           | Accept a request               |
 * | PUT    | id=xxx&action=reject      | Yes           | Reject a request               |
 * | PUT    | id=xxx&action=archive     | Yes           | Archive a request              |
 * | PUT    | id=xxx&action=restore     | Yes           | Restore archived request       |
 * | DELETE | id=xxx                    | Yes           | Delete a request               |
 * ---------------------------------------------------------
 * 
 * Request Status Flow:
...
module.exports = async function handler(req, res) {
  ...
  // Extract query parameters for routing
  const { id, action } = req.query;

  // ==================== CREATE NEW SOLICITUD (PUBLIC) ====================
  // POST /api/solicitudes
  // Allows patients to submit study requests without authentication
  // Links the request to the selected doctor via their email
