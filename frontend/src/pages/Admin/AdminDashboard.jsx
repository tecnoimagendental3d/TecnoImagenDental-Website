...
const CreateUserModal = ({ isOpen, onClose, onSave }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axiosInstance.post('/api/auth/admin/create-user', dataToSend);
...
const EditUserModal = ({ isOpen, onClose, user, onSave }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axiosInstance.put(`/api/auth/admin/user/${user._id}`, dataToSend);
...
const AdminDashboard = () => {
  ...
  const fetchDoctors = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await axiosInstance.get('/api/auth/admin/doctors');
  ...
  const handleChangeStatus = async (id, newStatus, name) => {
    ...
    try {
      setProcessingId(id);
      await axiosInstance.put(`/api/auth/admin/user/${id}/status`, { status: newStatus });
  ...
  const handleDelete = async (id, name) => {
    ...
    try {
      setProcessingId(id);
      await axiosInstance.delete(`/api/auth/admin/user/${id}`);
