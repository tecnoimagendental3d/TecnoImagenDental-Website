...
const getDashboardSummary = async (req, res) => {
  ...
  try {
    ...
    const confirmedInvoices = invoices.filter(inv => inv.status === 'Confirmado');
    const unconfirmedInvoices = invoices.filter(inv => inv.status !== 'Confirmado');
    const totalRevenue = confirmedInvoices.reduce((acc, inv) => acc + inv.total, 0);
    const totalOutstanding = unconfirmedInvoices.reduce((acc, inv) => acc + inv.total, 0);
    
    const dataSummary = `
      - Total number of invoices: ${totalInvoices}
      - Total confirmed invoices: ${confirmedInvoices.length}
      - Total unconfirmed invoices: ${unconfirmedInvoices.length}
      - Total revenue from confirmed invoices: ${totalRevenue.toFixed(2)}
      - Total outstanding amount from unconfirmed invoices: ${totalOutstanding.toFixed(2)}
